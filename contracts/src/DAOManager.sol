...
contract DAOManager {
    ...
    uint256 public s_totalDaos;
    uint256 public immutable i_contractCreationTime;
    uint256 public s_totalDocuments;
    ...
    constructor() {
        i_admin = msg.sender;
        i_contractCreationTime = block.timestamp;
    }
    ...
    function createUser(
        ...
        userIdtoUser[s_totalUsers] = u1;
        userWallettoUserId[_userWalletAddress] = s_totalUsers;

        emit UserRegistered(s_totalUsers, _userName, _userWalletAddress);
    }
    ...
    function createDao(
        ...
        daoIdtoDao[s_totalDaos] = d1;
        daoIdtoMembers[s_totalDaos].push(creatorId);
        userIdtoDaos[creatorId].push(s_totalDaos);

        emit DAOCreated(s_totalDaos, _daoName, _userWalletAddress);
    }
    ...
    function createProposal(
        ...
        DAOToken daoToken = DAOToken(daoGovernanceToken);
        require(
            daoToken.balanceOf(_userWalletAddress) >=
        ...
        proposalIdtoProposal[s_totalProposals] = p1;
        daoIdtoProposals[_daoId].push(s_totalProposals);

        emit ProposalCreated(s_totalProposals, _daoId, _userWalletAddress);
    }
    ...
    function joinDao(uint256 _daoId, address _callerWalletAddress) public {
        ...
        DAOToken daoToken = DAOToken(tempTokenAddress);
        uint256 userBalance = daoToken.balanceOf(_callerWalletAddress);
        require(
    ...
    function uploadDocument(
        ...
        documentIdtoDocument[s_totalDocuments] = d1;
        daoIdtoDocuments[_daoId].push(s_totalDocuments);

        emit DocumentUploaded(s_totalDocuments, _daoId, msg.sender);
    }
    ...
    function voteForProposal(
        ...
        address votingTokenAddress = proposalIdtoProposal[_proposalId]
            .votingTokenAddress;
        DAOToken daoToken = DAOToken(votingTokenAddress);
        uint256 userBalance = daoToken.balanceOf(msg.sender);
        uint256 tempUserId = userWallettoUserId[msg.sender];
        require(
    ...
    function qvVoting(
        ...
        daoToken.transferFrom(funcCaller, address(this), _numTokens);
    ...
    function hasVoted(
        ...
        uint256 totalVoters = proposalIdtoVoters[_proposalId].length;
